	  ...
	  if (match->type == ZEBRA_ROUTE_CONNECT)
		...
		if (CHECK_FLAG (newhop->flags, NEXTHOP_FLAG_FIB)
		    && ! CHECK_FLAG (newhop->flags, NEXTHOP_FLAG_RECURSIVE))
		  {
			...
			if (newhop->gate)
			  prefix_copy ((nexthop->rgate = prefix_new ()),
                                       newhop->gate);
  ...
  if (IS_ZEBRA_DEBUG_KERNEL && gate) 
    {
      ...
      zlog_debug ("rib_delete_ipv4(): route delete %s/%d via %s ifindex %d",
                  inet_ntop (AF_INET, &p->u.prefix, buf1, BUFSIZ),
                  p->prefixlen, 
                  buf2,
                  ifindex);
    }
 ...
 */
static int
static_same_target (struct static_route *s1, struct static_route *s2)
{
  ...
  if (s1->gate || s2->gate)
    {
      ...
      if (!prefix_same (s1->gate, s2->gate))
        return 0;
    }
  if (s1->ifname || s2->ifname)
    {
      if (!(s1->ifname && s2->ifname))
        return 0;
      
      if (strncmp (s1->ifname, s2->ifname, INTERFACE_NAMSIZ) != 0)
        return 0;
    }
            ...
            u_int32_t vrf_id)
{
  ...
  new_si.flags = flags;
  new_si.ifname = ifname;
  new_si.gate = gate;
  new_si.distance = distance;
  
  /* Do nothing if there is a same static route.  */
  for (si = rn->info; si; si = si->next)
    {
      if (static_same_target (si, &new_si))
	{
