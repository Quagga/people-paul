           ...
           nexthop = nexthop->next)
        {
          if ((cmd == RTM_NEWROUTE
                  ...
                  && CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_FIB)))
            {
              ...
              if (CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_RECURSIVE))
                {
                  if (IS_ZEBRA_DEBUG_KERNEL)
                    {
                      ...
                      prefix2str (nexthop->rgate, gbuf, sizeof(gbuf));
                  ...
                  assert (nexthop->rgate 
                          || nexthop->rifindex != IFINDEX_INTERNAL);
                  ...
                  if (IS_ZEBRA_DEBUG_KERNEL)
                    {
                      ...
                      prefix2str (p, pbuf, sizeof(pbuf));
                      prefix2str (nexthop->gate, gbuf, sizeof(gbuf));
              ...
              assert (gate || ifindex != IFINDEX_INTERNAL);
                  
              if (gate)
                {
                  rta_addattr_l (rta, 4096, RTA_GATEWAY,
                                 &gate->u.prefix,
                                 PREFIX_ADDR_BYTELEN(gate));
